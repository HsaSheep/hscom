...
##名称：アイテム
execute as @e[tag=shop_position] at @s run data modify entity @e[tag=shop_position,sort=nearest,limit=1] CustomName set value '{"text":"右クリックで購入","color":"#8B00A1","bold":false}'
...
#execute as @e[tag=shop_position] at @s positioned ~ ~-1 ~ run summon villager ~ ~ ~ {NoGravity:1b,Silent:1b,Invulnerable:1b,CustomNameVisible:0b,PersistenceRequired:1b,NoAI:1b,CanPickUpLoot:0b,Xp:251,Tags:["ww","shop"],CustomName:'{"text":"アイテム","color":"dark_purple","bold":true}'}
execute as @e[tag=shop_position] at @s positioned ~ ~-1 ~ run summon villager ~ ~ ~ {NoGravity:1b,Silent:1b,Invulnerable:1b,CustomNameVisible:1b,PersistenceRequired:1b,NoAI:1b,CanPickUpLoot:0b,Xp:251,Tags:["ww","shop"],CustomName:'{"text":"アイテム","color":"dark_purple","bold":true}'}
