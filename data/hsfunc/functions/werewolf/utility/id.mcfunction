...
execute unless entity @a[tag=wwp,scores={p_id=0}] run title @a[tag=wwps] actionbar ["",{"text":"--- [ID設定完了:"},{"score":{"name": "@e[tag=game,limit=1]","objective": "p_count"},"color":"gold"},{"text":"] ---"}]
