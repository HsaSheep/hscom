...
#更新、終了判定
execute if score @e[tag=DummyPlayer,tag=Timer,limit=1] timer_running matches 1 run execute if score @e[tag=DummyPlayer,tag=Timer,limit=1] timer_countdown matches 2.. run schedule function hsfunc:werewolf/timer/update/countdown 1s append
execute if score @e[tag=DummyPlayer,tag=Timer,limit=1] timer_running matches 1 run execute unless score @e[tag=DummyPlayer,tag=Timer,limit=1] timer_countdown matches 2.. run schedule function hsfunc:werewolf/timer/update/countdown_end 1s append