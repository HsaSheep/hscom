...
#終了判定
##runningフラグ確認-0判定
execute if score @e[tag=DummyPlayer,tag=Timer,limit=1] timer_running matches 1 run execute if score @e[tag=DummyPlayer,tag=Timer,limit=1] time matches 1.. run schedule function hsfunc:timer/update/timer_update 1s append
execute unless score @e[tag=DummyPlayer,tag=Timer,limit=1] time matches 1.. run function hsfunc:timer/update/timer_done