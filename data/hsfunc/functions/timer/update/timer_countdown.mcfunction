...
#更新、終了判定
execute if score @e[tag=DummyPlayer,tag=Timer,limit=1] timer_countdown matches 2.. run schedule function hsfunc:timer/update/timer_countdown 1s append
execute unless score @e[tag=DummyPlayer,tag=Timer,limit=1] timer_countdown matches 2.. run schedule function hsfunc:timer/update/timer_countdown_end 1s append